MODULE global
    ...
    DOUBLE PRECISION Y2model(500000),Y3model(500000),Rmodel(500000),Bwave(6)
...
PROGRAM leakage
      ...
      DOUBLE PRECISION energysp,vvnorm,vxpl,vypl,vzpl,vvnormpl,vperp,vpara
	  REAL PARMOD(10)
...
!        vstart(5)=Req*dcos(FAI*rad)	 !y
     ...
     &ticle'
       ...
       ! 200eV -- 206.393  pa=100.13769
       !150eV   --156.393  pa=101.66577
       !100eV   --106.393  pa=104.19088
       !50eV    --56.393   pa=109.6757

!        DO wnum=1,45,1
        DO wnum=1,300,1
            T_end=(wnum-1)*0.06+0.001
       !     T_end=10
            ttotal=T_end
        DO beta_=140,140,10
        !DO alpha_=10,90,10 
        DO alpha_=5,355,10
            ...
            alpha=alpha_
            energysp=0.039
            vvnorm=sqrt(2*energysp*1.d3*abs(qmin))
            ...
            energy=vvnormpl/2/abs(qmin)/1.d3
...
!  GSM coordinates !! //zhou 2002.10.10
	...
	COMMON /calcu/ fb,vol,vorp
      ...
      Z=posi(3)
      vphase=400*1d3
      zmax=(ttotal+t)*vphase/Re-240d4/Re
      zmin=((ttotal+t)*vphase-720d4)/Re
!      omega=2*pi*(0.1+(zamx-Z)*Re/480d4)
!      k=omega/vphase
!      zmiddle=(ttotal+t)*vphase/Re-280d4/Re
      omega=2*pi*0.1*exp((zmax-Z)*Re/200d4)
      k=omega/vphase
      zmiddle=(ttotal+t)*vphase/Re-350d4/Re
      bochang=100d4/Re
!      if (Z>zmin .and. Z<zmax) then
    !    if (Z<zmax) then
        Ef(1)=0.6*cos(-omega*(ttotal+t)+k*Z*Re-pi/2+pi)*exp(-(Z-zmiddle)**2/(bochang)**2)
        Ef(2)=0.6*sin(-omega*(ttotal+t)+k*Z*Re-pi/2+pi)*exp(-(Z-zmiddle)**2/(bochang)**2)
        Ef(3)=0.d0
    !  else
    !      Ef(1)=0.0
    !      Ef(2)=0.0
...
!  in GSM coordinates
	...
	COMMON /B_field/ B1,Bzimf,Vsw
      ...
      zmax=(ttotal+t)*vphase/Re-240d4/Re
      zmin=((ttotal+t)*vphase-720d4)/Re
 !     omega=2*pi*(0.1+(zmax-Z)*Re/480d4)
 !     k=omega/vphase
 !     zmiddle=(ttotal+t)*vphase/Re-280d4/Re
...
!      if (Z>zmin .and. Z<zmax) then
    !  if (Z<zmax) then
        Bf(1)=1.5d0*cos(-omega*(ttotal+t)+k*Z*Re+pi)*exp(-(Z-zmiddle)**2/(bochang)**2)
        Bf(2)=1.5d0*sin(-omega*(ttotal+t)+k*Z*Re+pi)*exp(-(Z-zmiddle)**2/(bochang)**2)
        Bf(3)=2.d0
    !  else
    !      Bf(1)=0.0
    !      Bf(2)=0.0
