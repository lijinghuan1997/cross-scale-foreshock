...
!    data Bwave /1.6,2.5,2.1,2.7,3.0,2.1/
    data Bwave /3.5,3.5,3.5,3.5,3.5,3.5/
    !data EN /29,31,33,38,40,43/
    data EN /17.2,18.5,19.8,22.5,24,25.5/
    DOUBLE PRECISION pi,Re,lamda,v0
    DOUBLE PRECISION B00,mmm,dir
    DOUBLE PRECISION eq,ttotal,eq2,phase,vvperp,xc0
    INTEGER wnum
    END MODULE
...
!        vstart(5)=Req*dcos(FAI*rad)	 !y
     ...
     &ticle'
       ...
       DO wnum=1,151,1
            ...
            T_end=(wnum-1)*3+0.001
            ttotal=T_end
        DO alpha_=15,345,30
        !DO alpha_=195,195,30
            alpha=alpha_
           ! energysp=14.2*1.3023**(Ennum-5)/1.d3
       
            energysp=3
            beta=beta_
            vvnorm=sqrt(2*energysp*1.d3*qmin)
            vvectorsp(1)=sin(real(beta)/180.0*pi)*cos(real(alpha)/180.0*pi)
            vvectorsp(2)=sin(real(beta)/180.0*pi)*sin(real(alpha)/180.0*pi)
            vvectorsp(3)=cos(real(beta)/180.0*pi)
            plx=0.d0 !-10.d3
            ply=0.d0 !-40.d3
            plz=0.d0 !35.d3
            vxpl=vvnorm*vvectorsp(1)-plx
            vypl=vvnorm*vvectorsp(2)-ply
            vzpl=vvnorm*vvectorsp(3)-plz
!            vzpl=40.d3*(real(beta_)-20.0)
            vvnormpl=vxpl**2+vypl**2+vzpl**2
            vvperp=sqrt(vxpl**2+vypl**2)
   !         xc0=vvperp/qmin/3.0*1.d9*cos((real(alpha)-90)/180.0*pi)
            energy=vvnormpl/2/qmin/1.d3
            vvector(1)=vxpl/sqrt(vvnormpl)
            vvector(2)=vypl/sqrt(vvnormpl)
            vvector(3)=vzpl/sqrt(vvnormpl)
            Xstart=0.d0/Re  !-plx*T_end/Re;
            Ystart=0.d0/Re  !-ply*T_end/Re;      
	        Zstart=0.d0/Re  !-plz*T_end/Re;
            vstart(4)=Xstart
	        vstart(5)=Ystart
            vstart(6)=Zstart
...
!  GSM coordinates !! //zhou 2002.10.10
	...
	DOUBLE PRECISION R,angle,X,Y,Z,interp,omega,vphasez,vphasex,kz,kx,zmin,zmax,zmiddle,bochang,Ezhenfu,WNA,knorm
    COMMON /basics/ qm,rad,E0,B0,vc,qmsign
    COMMON /B_field/ B1,Bzimf,Vsw
	COMMON /ele/ Einv
	COMMON /calcu/ fb,vol,vorp
      ...
      zmax=((ttotal+t)*vphase-10*vphase*45.d0)/Re
      zmiddle=(zmin+zmax)/2.0
      bochang=abs(vphase)*45.d0/Re
        Ef(1)=0.9*cos(-omega*(ttotal+t)+k*Z*Re-pi/2)*exp(-(Z-zmiddle)**2/(2*bochang)**2)
        Ef(2)=0.9*sin(-omega*(ttotal+t)+k*Z*Re-pi/2)*exp(-(Z-zmiddle)**2/(2*bochang)**2)
        Ef(3)=0.d0
...
!  in GSM coordinates
      ...
      REAL PARMOD(10)
	DOUBLE PRECISION L,interp,omega,vphasez,vphasex,kz,kx,zmin,zmax,zmiddle,bochang,WNA,knorm
	...
	COMMON /B_field/ B1,Bzimf,Vsw
      ...
      zmiddle=(zmin+zmax)/2.0
      bochang=abs(vphase)*45.d0/Re
        Bf(1)=2.d0*cos(-omega*(ttotal+t)+k*Z*Re)*exp(-(Z-zmiddle)**2/(2*bochang)**2)
        Bf(2)=2.d0*sin(-omega*(ttotal+t)+k*Z*Re)*exp(-(Z-zmiddle)**2/(2*bochang)**2)
        Bf(3)=3.d0
...
!      每outputonce步输出一次 //zhou
   ...
   !  &enem,pitchangle  !,fai,LATIT,SIGMA2,ppwhere
      if(abs(t+ttotal).LE.0.01) then
        write(40,20) t,v(1),v(2),v(3),enem,Bf(1),Bf(2)
      end if
    end if
15      format(5x,3(A,f10.6))
